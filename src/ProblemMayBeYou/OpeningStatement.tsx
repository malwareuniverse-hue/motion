import React from "react";
import { useCurrentFrame } from "remotion";
import { palette } from "./theme";
import { poppins } from "./fonts";
import { clampedInterp, easeOutCubic, fadeSlide } from "./utils";
import { OPENING_ACCENT, OPENING_LEAD, T } from "./timeline";

export const OpeningStatement: React.FC = () => {
  const frame = useCurrentFrame();

  const enter = fadeSlide(frame, T.openingIn, T.openingDur, 20);
  const recede = clampedInterp(
    frame,
    [T.openingRecedeStart, T.openingRecedeStart + T.openingRecedeDur],
    [0, 1],
    easeOutCubic,
  );

  return (
    <div
      style={{
        position: "absolute",
        inset: 0,
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        padding: "0 130px",
        opacity: enter.opacity * (1 - recede),
        transform: `translateY(${enter.translateY - recede * 30}px) scale(${1 - recede * 0.03})`,
        filter: `blur(${recede * 3}px)`,
      }}
    >
      <span
        style={{
          maxWidth: "86%",
          fontFamily: poppins,
          fontSize: 44,
          fontWeight: 700,
          lineHeight: 1.32,
          letterSpacing: 0.4,
          color: palette.softWhite,
          textAlign: "center",
        }}
      >
        {OPENING_LEAD}
        <span style={{ color: palette.warmGold }}>{OPENING_ACCENT}</span>
      </span>
    </div>
  );
};
